...
<div class="row mb-4">
    <div class="col-12 text-center">
        <h1 class="mb-4 fw-bold text-secondary"><i class="fa-solid fa-mountain"></i> Nadmořská výška</h1>
    </div>
</div>

<div class="card shadow-sm mt-4 border-secondary bg-black">
    <div class="card-header bg-secondary d-flex flex-column flex-md-row justify-content-between align-items-md-center py-3 gap-3">
        <h5 class="mb-0 text-white fw-bold"><i class="fa-solid fa-chart-line me-2"></i></h5>
    ...
    <div class="card-body p-0" style="background-color: #1d1d1d;">
        <canvas id="vyskaGraf" style="min-height: 350px;"></canvas>
    </div>
</div>

<div class="row justify-content-center mt-4"> 
    <div class="col-md-12">
        <div class="card bg-dark text-light mb-3 text-center border-secondary">
            <div class="card-body">
                <h6 id="titulekPrumer" class="text-uppercase fw-bold text-secondary" style="letter-spacing: 1px;">Průměr (1h)</h6>
...
<style>
    ...
    .btn-group .btn-dark:hover, .btn-group .btn-dark.active { background-color: #545b62 !important; border-color: #545b62 !important; color: #ffffff !important; }
...
<script>
    ...
    const ctx = document.getElementById('vyskaGraf').getContext('2d');
    const chart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: [], 
            datasets: [{
                label: 'Výška (<?= $jednotka ?>)',
                data: [], 
                borderColor: '<?= $barva ?>', 
    ...
    function nactiData(obdobi, tlacitko = null) {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                chart.data.datasets[0].label = `Výška (Poslední hodnota: ${posledniHodnota} <?= $jednotka ?>)`;
                chart.update();
