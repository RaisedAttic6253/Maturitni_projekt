...
<div class="row mb-4">
    <div class="col-12 text-center">
        <h1 class="mb-4 fw-bold text-primary"><i class="fa-solid fa-droplet"></i> Relativní vlhkost vzduchu</h1>
    </div>
</div>

<div class="card shadow-sm mt-4 border-primary bg-black">
    <div class="card-header bg-primary d-flex flex-column flex-md-row justify-content-between align-items-md-center py-3 gap-3">
    ...
    <div class="card-body p-0" style="background-color: #1d1d1d;">
        <canvas id="vlhkostGraf" style="min-height: 350px;"></canvas>
    </div>
</div>

<div class="row justify-content-center mt-4"> 
    <div class="col-md-12">
        <div class="card bg-dark text-light mb-3 text-center border-primary">
            <div class="card-body">
                <h6 id="titulekPrumer" class="text-uppercase fw-bold text-primary" style="letter-spacing: 1px;">Průměr (1h)</h6>
...
<style>
    ...
    .btn-group .btn-dark:hover, .btn-group .btn-dark.active { background-color: #0056b3 !important; border-color: #0056b3 !important; color: #ffffff !important; }
...
<script>
    ...
    const ctx = document.getElementById('vlhkostGraf').getContext('2d');
    const chart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: [], 
            datasets: [{
                label: 'Vlhkost (<?= $jednotka ?>)',
                data: [], 
                borderColor: '<?= $barva ?>', 
    ...
    function nactiData(obdobi, tlacitko = null) {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                chart.data.datasets[0].label = `Vlhkost (Poslední hodnota: ${posledniHodnota} <?= $jednotka ?>)`;
                chart.update();
